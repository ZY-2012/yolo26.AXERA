...
**结论：混合量化不会把算子放到 CPU。** 三个模型编译后都只有 1 个 NPU 子图、0 个 CPU 子图；板端 CPU 占用只由「每帧固定的 CPU 侧开销 ÷ 单帧耗时」决定，与量化精度分配无关。
...
**关键分析**：CPU 侧固定开销 = Python 延迟 − `ax_run_model` 原生延迟 ≈ **4.8 ms**（4.81 / 4.81 / 4.70），进程 CPU% ≈ 4.8 / 单帧耗时：
