...
2. **板端 CPU 占用与是否混合量化无关，只与“每帧固定的 CPU 侧开销 ÷ 单帧耗时”成正比**：
   三种模型的 Python 与 `ax_run_model` 原生延迟差都是 **~4.8 ms**（CPU 侧固定开销），
   进程 CPU% ≈ 4.8 / 延迟，实测 43.2% / 23.6% / 34.7% 完全吻合。
   混合模型 CPU 介于 U8/U16 之间，是因为它比 U16 快、同时又比 U8 慢，不是因为它多了 CPU 算子。
3. 参考截图 `CPU usage 54.9% @ 47.6ms` 换算约 **26 ms/帧的 CPU 工作量**，远超本复现的 ~4.8 ms，
...
- CPU 侧固定开销 = Python avg − 原生 avg：U8 4.81 ms / U16 4.70 ms / 混合 4.81 ms。
- 进程 CPU% ≈ CPU 侧开销 / 帧耗时 × 100%：
...
> 板端 CPU 占用主要来自推理循环里每帧固定的 CPU 侧开销（输入准备、输出 dequant/搬运、Python/axengine 框架开销，约 4.8 ms/帧），以及 demo 自身做的后处理。模型越快（U8 > 混合 > U16），每秒处理的帧数越多，CPU 占用反而越高：实测进程 CPU 分别为 42.7% / 34.7% / 23.7%。
